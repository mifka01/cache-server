...
if [[ ! -f "$STATE_FILE" ]]; then
  rm -f "$STATE_DIR/vde.pid"
  rm -rf "$STATE_DIR/vde1.ctl" "$STATE_DIR"/vm-state-*
  echo "No running lab session found. Cleaned stale runtime files."
  exit 0
fi
...
rm -f "$STATE_DIR/vde.pid"
rm -rf "$STATE_DIR/vde1.ctl" "$STATE_DIR"/vm-state-*
echo "Lab processes stopped and state cleaned."
